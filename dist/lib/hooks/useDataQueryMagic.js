...
export const useDataQueryMagic = () => {
    const { mutateQuery, notifyInvalidation, notifyCancellationOfQuery } = useDataQueryContext();
    /**
     ...
     * @param {*} dataQueryKey
     */
    const invalidateQuery = useCallback((dataQueryKey) => {
        if (!dataQueryKey)
     ...
     */
    const invalidateQueries = useCallback((dataQueryKeys) => {
     ...
     */
    const setQueryData = useCallback((dataQueryKey, querySetter) => {
        if (!dataQueryKey)
        ...
        if (typeof querySetter === "function") {
            ...
            data = querySetter;
        if (mutateQuery)
            mutateQuery(dataQueryKey.toString(), data);
    }, [mutateQuery]);
    /**
    ...
    const setQueriesData = useCallback((dataQueries) => {
        if (!Array.isArray(dataQueries))
            throw new TypeError("Please provide an array of dataQuery objects in which must contain dataQueryKey and querySetter!");
     ...
     */
    const getQueryData = useCallback((dataQueryKey) => {
        if (!dataQueryKey)
     ...
     */
    const getQueriesData = useCallback((dataQueryKeys) => {
        if (!Array.isArray(dataQueryKeys))
     ...
     */
    const cancelQuery = useCallback(async (dataQueryKey) => {
        if (!dataQueryKey)
            throw new TypeError("Don't provide dataQueryKey as falsy value!");
     ...
     * @param {*} dataQueryKey
     */
    const clearCacheData = useCallback((dataQueryKey) => {
        if (!dataQueryKey)
            throw new TypeError("Don't provide dataQueryKey as falsy value!");
